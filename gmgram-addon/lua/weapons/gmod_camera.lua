...
SWEP.Author = "Garry(lua) + XxLMM13xXgaming(gmgram) + Segeco(gmgram)"
...
function SWEP:Reload()
	...
	self:SetRoll( 0 )
	if self.Owner:GetNWBool("GMGramGetCameraModeCanSwitch") == nil then
		self.Owner:SetNWBool("GMGramGetCameraModeCanSwitch", true)
	end

	if self.Owner:GetNWBool("GMGramGetCameraModeCanSwitch") == false and self.Owner:GetNWBool("GMGramGetCameraModeCanSwitch") != nil then
		return
	end
	...
	if self.Owner:GetNWBool("GMGramGetCameraModeCanSwitch") then
		if self.Owner:GetNWFloat("GMGramGetCameraMode") == 0 then
			self.Owner:SetNWBool("GMGramGetCameraModeCanSwitch", false)
			self.Owner:SetNWFloat("GMGramGetCameraMode",1)
			if CLIENT then
				chat.AddText(Color(0,0,0), "[", Color(255,255,255), "GMGram", Color(0,0,0), "] ", Color(0,0,0), "Camera mode set to GMGram Upload!")
			end
		elseif self.Owner:GetNWFloat("GMGramGetCameraMode") == 1 then
			self.Owner:SetNWBool("GMGramGetCameraModeCanSwitch", false)
			self.Owner:SetNWFloat("GMGramGetCameraMode",0)
			if CLIENT then
				chat.AddText(Color(0,0,0), "[", Color(255,255,255), "GMGram", Color(0,0,0), "] ", Color(0,0,0), "Camera mode set to Steam Upload!")
			end
		end
	end
end
...
function SWEP:PrimaryAttack()
	...
	elseif self.Owner:GetNWFloat("GMGramGetCameraMode") == 1 then
		...
		if ( !game.SinglePlayer() && SERVER ) then return end
		if ( CLIENT && !IsFirstTimePredicted() ) then return end
		self.Owner:ConCommand( "gmgram_takepicture" )
	end
end
...
--  self.Owner:ChatPrint("Selfie coming soon?")
end
...
function SWEP:Deploy()
	if (CLIENT) then
		print("Welcome to GMGram")
		chat.AddText(Color(0,0,0), "[", Color(255,255,255), "GMGram", Color(0,0,0), "] ", Color(0,0,0), "Type !gmgram for more info on uploading pictures!")
	end
	return true
end
